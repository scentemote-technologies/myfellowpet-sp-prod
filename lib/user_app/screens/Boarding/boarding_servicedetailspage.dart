...
import '../../main.dart';
import '../Authentication/FirstTimeUserLoginDeyts.dart';
...
''';
        ...
        return PopScope(
          // 1. Set canPop to true to allow the system to pop for normal navigation.
          canPop: true,

          onPopInvokedWithResult: (didPop, result) async {
            // If the system successfully popped (didPop is true), do nothing.
            if (didPop) return;

            // If didPop is false, it means we are at the root. We must check auth and redirect.
            ...
            if (!userDoc.exists) {
              Navigator.pushAndRemoveUntil(
                context,
                MaterialPageRoute(
                  builder: (_) => UserDetailsPage(phoneNumber: user.phoneNumber ?? ""),
                ),
                    (route) => false,
              );
              return;
            ...
            // 🚀 If stack is empty (and we reached here), go to Boarding tab of HomeWithTabs
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => HomeWithTabs(
                initialTabIndex: 1, // Set to Boarding tab
              )),
            );
          },
