...
class _ConfirmationPageState extends State<ConfirmationPage> {
  ...
  Future<_FeesData> _fetchFees() async {
    // This is the same _fetchFees method from SummaryPage
    ...
    final platformFee =
        double.tryParse(data['user_app_platform_fee'] ?? '0') ?? 0;
  ...
  Future<String> _fetchPhoneNumber() async {
    final doc = await FirebaseFirestore.instance
        ...
        .get();
    final data = doc.data();
    if (data == null || data['service_phone_number'] == null) {
      throw 'No Number';
    }
    return data['service_phone_number'] as String;
  }
  ...
  Future<String> _fetchWhatsappNumber() async {
    ...
    if (data == null || data['service_whatsapp_phone_number'] == null) {
      throw 'No Whatsapp';
    }
    return data['service_whatsapp_phone_number'] as String;
  }
...
// --- All the logic is now safely inside the builder ---
                ...
                final double subTotal = newBoardingCost +
                    (widget.foodCost ?? 0) +
                          ...
                          builder: (_, snap) {
                            ...
                            return Padding(
                              ...
                              child: Column(
                                ...
                                children: [
                                  _buildItemRow(
                                      'Boarding Fee', newBoardingCost),
                                  if ((widget.foodCost ?? 0) > 0)
                                    _buildItemRow('Meal Fee', widget.foodCost!),
                                  if ((widget.walkingCost ?? 0) > 0)
                                    _buildItemRow(
                                        'Walking Fee', widget.walkingCost!),
                                  ...
                                  const Padding(
                                      ...
                                      child: DottedDivider()),
                                  _buildItemRow('Grand Total', grandTotal,
                                      isTotal: true),
...
// This now matches the logic in your working _bookingDetailsContent widget.
    ...
    return Column(
      children: List.generate(widget.petIds.length, (index) {
        ...
        final petName = widget.petNames[
        index]; // <-- THE FIX: Get the name from the correct list.
...
// A safety check in case there are no service details for a pet.
        ...
        final double walkingRate =
        (widget.walkingRates[petSize] ?? 0).toDouble();
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                'Pet: $petName ($petSize)',
                // Use the correct petName and petSize
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () {
              // TODO: Handle Help tap (e.g., open support chat or call)
            },
            ...
            Expanded(
              child: OutlinedButton( // Changed from TextButton to OutlinedButton
                style: OutlinedButton.styleFrom(
                  ...
                  foregroundColor: canCancel ? Colors.red.shade700 : Colors.grey,
                  side: BorderSide(
                    color: canCancel ? Colors.red.shade700 : Colors.grey.shade300,
                    width: 1.5,
                  ),
                  shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12)),
