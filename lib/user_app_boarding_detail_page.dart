...
class _BoardingServiceDetailPageState extends State<BoardingServiceDetailPage>
  ...
  Future<void> _fetchPetDetails(String petId) async {
    ...
    try {
      ...
      debugPrint("Error fetching pet details for $petId: $e");
      setState(() {
        _acceptedSizes = [];
        _acceptedBreeds = [];
      });
  ...
  Future<void> checkIfLiked(String serviceId) async {
    ...
    if (userPreferencesDoc.exists) {
      ...
      setState(() {
        ...
        debugPrint('Service $serviceId liked status: $isLiked');
      });
    } else {
      debugPrint('User preferences document does not exist. Creating a new one...');
      await userPreferencesRef.set({'liked': [serviceId]});
      setState(() {
        isLiked = true;
      });
            ...
            itemBuilder: (context, index) {
              return InteractiveViewer(
                child: CachedNetworkImage( // ✨ Use CachedNetworkImage
                  ...
                  fit: BoxFit.contain,
                  placeholder: (context, url) => Center(child: CircularProgressIndicator(color: Color(0xFF2CB4B6))),
