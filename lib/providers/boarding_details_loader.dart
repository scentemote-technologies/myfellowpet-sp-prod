...
class BoardingDetailsLoader extends StatefulWidget {
  ...
  const BoardingDetailsLoader({Key? key, required this.serviceId})
      : super(key: key);

  @override
  _BoardingDetailsLoaderState createState() => _BoardingDetailsLoaderState();
}

class _BoardingDetailsLoaderState extends State<BoardingDetailsLoader> {
  late Future<BoardingDetailsPage> _futurePage;
  ...
  Future<BoardingDetailsPage> _loadAllData(String id) async {
    // Step 1: Get the page data (await will pause execution here)
    final boardingPage = await _loadPage(id);
  ...
  Future<BoardingDetailsPage> _loadPage(String id) async {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<BoardingDetailsPage>(
        future: _futurePage,
        builder: (ctx, snap) {
